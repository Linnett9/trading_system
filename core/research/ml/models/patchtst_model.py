...
def _torch_dependencies() -> tuple[Any, Any]:
    try:
        ...
        raise RuntimeError(
            "PatchTST research model requires PyTorch. Install it with: python -m pip install torch"
        ) from exc
    return torch, nn


class _PatchTSTClassifierModule:  # populated dynamically after torch import
    pass


class PatchTSTSequenceMLModel:
    """Small PatchTST-style sequence classifier for research/shadow use.

    This keeps the existing IMLModel contract used by MLExperimentRunner:
      fit(list[dict], list[int]) -> None
      predict_proba(list[dict]) -> list[float]

    It converts tabular historical feature rows into rolling sequences, splits each
    sequence into temporal patches, embeds patches, applies a TransformerEncoder,
    and classifies the final pooled representation.
    """
    ...
    def __init__(
        ...
        if patch_length < 1:
            raise ValueError("patch_length must be at least 1")
        if patch_stride < 1:
            raise ValueError("patch_stride must be at least 1")
        if d_model % nhead != 0:
    ...
    def feature_importances(self) -> dict[str, float]:
        # PatchTST is not directly interpretable in the same way as a linear/tree model.
        return {}
    ...
    @classmethod
    def load(cls, path: Path) -> "PatchTSTSequenceMLModel":
        torch, nn = _torch_dependencies()
        ...
        if state_dict is not None and model.feature_names:
            model.model = _build_patchtst_module(
                torch=torch,
                nn=nn,
...
def _build_patchtst_module(
    ...
    class PatchTSTClassifier(nn.Module):
        def __init__(self) -> None:
            super().__init__()
            self.patch_length = min(patch_length, sequence_length)
            self.patch_stride = patch_stride
            self.n_patches = max(1, 1 + (sequence_length - self.patch_length) // self.patch_stride)
            self.patch_dim = self.patch_length * feature_count
        ...
        def forward(self, x: Any, padding_mask: Any | None = None) -> Any:
            # x: [batch, sequence_length, feature_count]
            ...
            if padding_mask is not None:
                ...
                x = x * valid
                mask_patches = padding_mask.unfold(dimension=1, size=self.patch_length, step=self.patch_stride)
                patch_padding_mask = mask_patches.all(dim=2)
            patches = x.unfold(dimension=1, size=self.patch_length, step=self.patch_stride)
            # unfold returns [batch, n_patches, feature_count, patch_length]
            patches = patches.transpose(2, 3).contiguous()
            patches = patches.reshape(patches.shape[0], patches.shape[1], self.patch_dim)
            tokens = self.patch_projection(patches) + self.position[:, : patches.shape[1], :]
            encoded = self.encoder(tokens, src_key_padding_mask=patch_padding_mask)
