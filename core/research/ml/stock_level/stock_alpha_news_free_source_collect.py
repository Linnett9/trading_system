...
from core.research.ml.stock_level.stock_alpha_news_free_source_collect_diagnostics import (
    _duplicate_diagnostics,
    _existing_output_overlap,
    _filter_rows_to_publication_window,
    _out_of_window_totals,
    _row_diagnostics,
)
...
def write_stock_alpha_news_free_source_collect(config: Mapping[str, Any], *, sources: Mapping[str, Any] | None = None) -> StockAlphaNewsFreeSourceCollectPaths:
    ...
    sec_event_rows = _sec_company_filing_event_rows(
        payload.pop("_sec_company_filings_event_source_rows", rows)
    )
...
def build_stock_alpha_news_free_source_collect(config: Mapping[str, Any], *, sources: Mapping[str, Any] | None = None) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    extended_batch_diagnostics: list[dict[str, Any]] = []
    provider_normalized_counts: dict[str, int] = {}
    sec_company_filings_event_source_rows: list[dict[str, Any]] = []
    ...
    for name, provider_config in provider_settings.items():
        ...
        counts[name] = 0
        provider_normalized_counts[name] = 0
        ...
        for batch_index, batch_symbols in enumerate(batches):
            ...
            try:
                ...
                provider_normalized_counts[name] += len(canonical_rows)
                if name == "sec_company_filings":
                    sec_company_filings_event_source_rows.extend(canonical_rows)
                ...
                extended_batch_diagnostics.append(diagnostic)
                batch_diagnostics.append(_legacy_provider_batch_diagnostic(diagnostic))
        ...
        if provider_normalized_counts.get(name, 0) == 0 and name not in zero_row_reasons:
    ...
    payload["provider_batch_extended_diagnostics"] = extended_batch_diagnostics
    payload.update(_out_of_window_totals(extended_batch_diagnostics))
    payload.update(_rss_registry_diagnostics(settings, symbols, extended_batch_diagnostics))
    payload["registry_validation"] = registry_validation
    if sec_company_filings_event_source_rows:
        payload["_sec_company_filings_event_source_rows"] = sec_company_filings_event_source_rows
    payload["provider_zero_row_reasons"] = {
        name: reason for name, reason in sorted(zero_row_reasons.items())
        if provider_normalized_counts.get(name, counts.get(name, 0)) == 0
        or reason in {"rate_limited", "entitlement_error", "provider_error"}
    }
    ...
    payload["providers_returned_zero_rows"] = sorted(
        name
        for name, count in provider_normalized_counts.items()
        if count == 0
    )
...
def _payload(settings: Mapping[str, Any], output_path: Path, requested: list[str], attempted: list[str], skipped: list[str], failures: dict[str, str], counts: dict[str, int], total: int) -> dict[str, Any]:
    ...
    return {
        ...
        "provider_batch_diagnostics": [], "provider_batch_extended_diagnostics": [],
        "provider_zero_row_reasons": {},
...
def _legacy_provider_batch_diagnostic(diagnostic: Mapping[str, Any]) -> dict[str, Any]:
    if str(diagnostic.get("provider", "")) == "alpaca_benzinga":
        legacy = dict(diagnostic)
        if legacy.get("termination_reason") == "rate_limited":
            legacy["termination_reason"] = "rate_limit"
        return legacy
    ...
    legacy = {key: diagnostic.get(key) for key in legacy_keys}
    for key in (
        "feed_diagnostics",
        "query_terms",
        "skipped_symbols",
