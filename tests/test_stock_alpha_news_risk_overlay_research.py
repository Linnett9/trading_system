...
def test_news_risk_overlay_research_writes_end_to_end_outputs(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert result.artifact_manifest_json_path.exists()
    assert result.artifact_validation_report_json_path.exists()
    assert result.news_validation_workflow_map_json_path.exists()
    assert result.artifact_lineage_report_json_path.exists()
