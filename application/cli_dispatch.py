...
def dispatch(args, config, feed):
    ...
    if args.mode == "ml-selector-target-tournament":
        ...
        return
    if args.mode == "ml-selector-cost-aware-policy-evaluation":
        _commands("ml_commands").run_ml_selector_cost_aware_policy_evaluation(config)
        return
    ...
    if args.mode == "ml-stock-alpha-news-source-setup-check":
        ...
        return
    if args.mode == "ml-stock-alpha-finbert-news-probe":
        _commands("ml_commands_stock").run_ml_stock_alpha_finbert_news_probe(config)
        return
